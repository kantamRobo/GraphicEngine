...
inline void RenderContext::SetComputeDescriptorHeap(std::shared_ptr<DescriptorHeap> descHeap)
{
	m_descriptorHeaps[0] = descHeap->get();
	m_commandList->SetDescriptorHeaps(1, m_descriptorHeaps->GetAddressOf());
...
inline void RenderContext::WaitUntilFinishDrawingToRenderTarget(std::shared_ptr<RenderTarget> renderTarget)
{
...
inline void RenderContext::WaitUntilToPossibleSetRenderTargets(int numRt, std::shared_ptr<RenderTarget> renderTargets[])
{
	for (int i = 0; i < numRt; i++) {
		WaitUntilToPossibleSetRenderTarget(renderTargets[i]);
	}
}
inline void RenderContext::WaitUntilFinishDrawingToRenderTargets(int numRt, std::shared_ptr<RenderTarget> renderTargets[])
{
	for (int i = 0; i < numRt; i++) {
