...
namespace raytracing {
	...
	namespace BuildSubObjectHelper {
		...
		//ExportAssociationのサブオブジェクト作成の
		//ヘルパー
		struct ExportAssociationSubObject {
			void InitEASubObject(const WCHAR* exportNames[], uint32_t exportCount, const D3D12_STATE_SUBOBJECT* pSubobjectToAssociate) {
		...
		{
			void InitSCsubobject(uint32_t maxAttributeSizeInBytes, uint32_t maxPayloadSizeInBytes)
			{
	...
	/// </summary>
		...
		{
			HitGroupSubObject() {}
			void InitHGSubObject(const SHitGroup& hitgroup)
			{
				desc = {};
	...
	raytracing::PSO::RootSignatureDesc PSO::CreateRayGenRootSignatureesc()
	{
	...
	void PSO::InitRTPSO(DescriptorHeaps& descriptorHeaps)
	{
		...
		for (int i = 0; i < eHitGroup_Num; i++) {
			hitGroupSOs[i].InitHGSubObject(hitGroups[i]);
